...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		// initialize root
		if(!(nw.dict.isWord(word2) && nw.dict.isWord(word1))) {
			return null;
		}
		this.root = new WPTreeNode(word1, null);
		// create queue to explore, visited list
		LinkedList<WPTreeNode> queue = new LinkedList<WPTreeNode>();
		LinkedList<String> visited = new LinkedList<String>();
		queue.add(this.root);
		visited.add(word1);
		while(!queue.isEmpty() && !visited.getLast().equals(word2)) {
			WPTreeNode curr = queue.pop();
			List<String> neighbors = nw.distanceOne(curr.getWord(), true);
			for(String w : neighbors) {
				if(!visited.contains(w)) {
					WPTreeNode node = curr.addChild(w);
					queue.add(node);
